using AuthServiceIN6BV.Aplication.Interfaces;
...
public class PasswordHashService : IPasswordHashService
{
    ...
    public bool VerifyPassword(string password, string hashedPassword)
    {
        try
        {
            ...
            {
                Console.WriteLine("[DEBUG] Using Argon2 standar format verification");
        ...
        {
            Console.WriteLine($"[DEBUG] Excpetion in VerifyPassword: {ex.Message}");
            return false;
        }
    }

    public bool VerifyArgon2StandardFormat(string password, string hashedPassword)
    {
        try
        {
